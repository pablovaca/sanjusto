...
var RadarChart = {
  draw: function(id, data, options) {
    ...
    /****************************************
    **-Functions to custom D3 radar chart-**
    ***************************************/
    ...
    function buildVisComponents() {
      ...
      vis.svg = d3.select(id)
        ...
        .attr("height", config.h + config.paddingY/2)
        .append("svg:g")
    ...
    function buildLevelsLabels() {
      for (var level = 0; level < config.levels; level++) {
        ...
        vis.levels
          ...
          .text((config.maxValue * (level) / config.levels).toFixed())
    ...
    function buildPolygons(data) {
      vis.vertices
        ...
        .attr("stroke", "#00acae") //custom color
    ...
    function verticesTooltipShow(d) {
      vis.verticesTooltip.style("opacity", 0.9)
        .html(d.fullName + "<br/><strong>Total Stars</strong>: " + (d.value-1))
